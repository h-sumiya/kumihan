...
import '../engine/table_renderer.dart';
import '../kumihan_controller.dart';
import '../kumihan_types.dart';
import '../parsers/aozora/ast.dart';
import 'ast_document_compiler.dart';

part 'kumihan_ast_page_renderer.dart';
...
class KumihanAstEngine implements LayoutEnvironment, KumihanViewport {
  KumihanAstEngine({
  ...
  Future<void> openAst(AozoraData data) async {
    final compiled = compileAozoraAst(data);
  ...
  void _handleAstCommandEntry(AstCommandEntry entry, double pageBlockSize) {
    ...
    switch (entry.kind) {
      ...
      case AstCommandKind.bottomAlignStart:
        _bottomMargin = entry.bottomAlignKind == AozoraBottomAlignKind.bottom
            ? 0
      ...
      case AstCommandKind.headingStart:
        ...
        _currentFontSize = switch (entry.headingLevel) {
          AozoraHeadingLevel.large => _fontSize * _fontScaleL * _fontScaleL,
          AozoraHeadingLevel.medium => _fontSize * _fontScaleL,
          _ => _fontSize,
        };
      ...
      case AstCommandKind.pageBreak:
        switch (entry.pageBreakKind) {
          case AozoraPageBreakKind.kaidan:
          case AozoraPageBreakKind.kaipage:
            if (!pageEmpty) {
              _pages.add(PageInfo(line: _lines.length));
            }
          case AozoraPageBreakKind.kaicho:
          ...
          case AozoraPageBreakKind.kaimihiraki:
  ...
  void _applyStyle(
    ...
    switch (style.kind) {
      ...
      case AstStyleKind.fontScale:
        ...
        final size = style.fontScaleDirection == AozoraFontScaleDirection.larger
